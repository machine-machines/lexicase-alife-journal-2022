...
namespace xeres {
    ...
    FloorIsLavaTaskWorld::FloorIsLavaTaskWorld(Agent* a, int environment) {

        // double angle = (0.01 * environment * M_PI) - (M_PI / 2.0);

        double angle = 0.0;

        total_ticks_in_the_air = 0;
        ...
        double grav_scale = static_cast<double>(environment) / 100.0;

        dWorldSetGravity(d_world, 0, 0, -10.0 * grav_scale);
    ...
    FloorIsLavaTaskWorld::~FloorIsLavaTaskWorld() {
    ...
    void FloorIsLavaTaskWorld::callback(dGeomID o1, dGeomID o2) {

        total_ticks++;

        if( o1!=ground && o2!=ground ) {
            total_ticks_in_the_air++;
            return;
        }
    ...
    void filtw_nearCallback (void *data, dGeomID o1, dGeomID o2)
    {
        ((FloorIsLavaTaskWorld*)data)->callback(o1, o2);
    }

    void FloorIsLavaTaskWorld::update() {

        agents[0]->tick(TIMESTEP);

        dSpaceCollide(d_space,this,&filtw_nearCallback);
        dWorldStep(d_world,TIMESTEP);
        dJointGroupEmpty(d_cngrp);
    }

    double FloorIsLavaTaskWorld::getFitness() {
        // bool reached_target = false;
        // double dist = 0.0;
        while( simTime < EVTIME ) {
            update();
        ...
        agents[0]->deactivate();
        double fitness = static_cast<double>(total_ticks_in_the_air) / static_cast<double>(total_ticks);
    ...
    vector<simbox> FloorIsLavaTaskWorld::getActiveBoxes() {
        std::vector<simbox> r;
